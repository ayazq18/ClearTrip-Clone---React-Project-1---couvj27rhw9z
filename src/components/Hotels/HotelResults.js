...
import CardCarausal from '../Corousel/HotelPage/CardCarausal'
...
const HotelResults = () => {
    ...
    const fetchHotels = async () => {
        try {
            const response = await fetch(`https://academics.newtonschool.co/api/v1/bookingportals/hotel?search={"location":"${inputResult}"}`, { method: "GET", headers: { Authorization: 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY1YWJlZWE2ZWM3MjNmN2NkZTA0OTJmNSIsImlhdCI6MTcwNTkxNDQyMywiZXhwIjoxNzM3NDUwNDIzfQ.NsXu4O1WNOfj__A2bSWNhgoazcYlUFMaWeMDp_fPTow', projectID: Project_ID, "Content-Type": "application/json" } });
            const result = await response.json()
            setHotelResultData(result.data.hotels)
                            ...
                            <div key={index} className='hotelMain'>
                                ...
                                <div className='hotelMain-sec3'>
                                    <div className='hotelMain-sec3-container1 flexY'>
                                        ...
                                        <p className='contain2'>+ {((item.avgCostPerNight*12)/100).toString().match(/^(\d+)(?=\.)/)[0]} tax</p>
                                        <p className='contain3'>/ night</p>
                                    </div>
                                    <div className='hotelMain-sec3-container2 flexY'>
                                        <del className='contain1'>₹4,143,601</del>
                                        <p className='contain2'>+ Additional bank discounts</p>
                                    </div>
                                </div>
                            </div>
                        ))}
